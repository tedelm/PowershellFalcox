...
Created: 2020-07-01
Edited: 
2020-07-01: Added function variables
...
#>
param (
    [Parameter()][Switch]$HTAViewCurrentConfig,
    [Parameter()][Switch]$HTACreateHTMLReport,
    ...
    [Parameter()][Switch]$HTARxDJI_SBUS_TX1,
    [Parameter()][Switch]$HTARxFLYSKY_IBUS_TX1,
    [Parameter()][Switch]$HTAESC_TELEM_UART6,
...
#Show Usage
#iwr -Uri https://softwaist.se/FalcoXLovesPowershell.php?Tell=yes | Out-Null
#[CmdletBinding()]
...
if($HTACreateBackup){
    ...
    $Outputfilepath = $DesktopFolder + "\MyFalcoXBackup.txt"
    Get-FalcoXConfig -comPort "$($MostLikelyCOMport)" -Dump -Outputfile "$($Outputfilepath)"
...
If($HTARxCRSF_TX1){
    ...
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 1 -SetUARTProtocol $(Get-UARTMapping -protocol CRSF)
}
If($HTARxFRSKY_INV_SBUS_TX1){
    #Get-UARTMapping -protocol INV_SBUS
    Write-Host "Setting TX1 to INV_SBUS"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 1 -SetUARTProtocol $(Get-UARTMapping -protocol INV_SBUS)
}
If($HTARxFRSKY_INV_FPORT_TX1){
    #Get-UARTMapping -protocol INV_FPORT
    Write-Host "Setting TX1 to INV_FPORT"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 1 -SetUARTProtocol $(Get-UARTMapping -protocol INV_FPORT)
}
If($HTARxDJI_SBUS_TX1){
    #Get-UARTMapping -protocol DJI_SBUS
    Write-Host "Setting TX1 to DJI_SBUS"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 1 -SetUARTProtocol $(Get-UARTMapping -protocol DJI_SBUS)
}
If($HTARxFLYSKY_IBUS_TX1){
    #Get-UARTMapping -protocol IBUS
    Write-Host "Setting TX1 to IBUS"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 1 -SetUARTProtocol $(Get-UARTMapping -protocol IBUS)
}
If($HTAVTX_SA_TX3){
    #Get-UARTMapping -protocol SMART_AUDIO
    Write-Host "Setting TX3 to SMART_AUDIO"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 3 -SetUARTProtocol $(Get-UARTMapping -protocol SMART_AUDIO)
}
If($HTAVTX_TRAMP_TX3){
    #Get-UARTMapping -protocol TRAMP_TELEM
    Write-Host "Setting TX3 to TRAMP_TELEM"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 3 -SetUARTProtocol $(Get-UARTMapping -protocol TRAMP_TELEM)
}
If($HTAESC_TELEM_UART6){
    #Get-UARTMapping -protocol BLHELI_TELEM
    Write-Host "Setting UART6 to BLHELI_TELEM"
    Setup-FalcoX -comPort $MostLikelyCOMport -SetUART 6 -SetUARTProtocol $(Get-UARTMapping -protocol BLHELI_TELEM)
